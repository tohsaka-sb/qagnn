...
def load_concept_embeddings(paths):
    arrays = [np.load(path) for path in paths]
    if len(arrays) == 1:
        return torch.as_tensor(arrays[0], dtype=torch.float)
    return torch.as_tensor(np.concatenate(arrays, 1), dtype=torch.float)
...
def load_resume_checkpoint(path):
    ...
    if isinstance(checkpoint, (list, tuple)) and len(checkpoint) >= 1:
        return {
            ...
            'args': checkpoint[1] if len(checkpoint) > 1 else None,
        }, False
    raise ValueError(f'Unsupported checkpoint format: {path}')


def save_resume_checkpoint(path, model, args, epoch_id, global_step, best_dev_acc, final_test_acc, best_dev_epoch):
    torch.save({
...
def train(args):
    ...
    best_model_path = os.path.join(args.save_dir, 'model.pt.best')
    checkpoint_last_path = os.path.join(args.save_dir, 'checkpoint_last.pt')
    log_path = os.path.join(args.save_dir, 'log.csv')
    is_resuming = args.resume_checkpoint_path is not None
    ...
    #   Load data                                                                                     #
    ###################################################################################################
    cp_emb = load_concept_embeddings(args.ent_emb_paths)
    ...
    if True:
        ...
        if args.load_model_path:
            ...
            model_state_dict, old_args = torch.load(args.load_model_path, map_location=torch.device('cpu'), weights_only=False)
            model.load_state_dict(model_state_dict)

        resume_state = None
        resumed_with_metadata = False
        start_epoch = 0
        global_step = 0
        best_dev_epoch = 0
        best_dev_acc, final_test_acc, total_loss = 0.0, 0.0, 0.0
        if args.resume_checkpoint_path:
            print(f'resuming training from {args.resume_checkpoint_path}')
            resume_state, resumed_with_metadata = load_resume_checkpoint(args.resume_checkpoint_path)
            model.load_state_dict(resume_state['model_state_dict'])
            if resumed_with_metadata:
                start_epoch = resume_state.get('epoch_id', -1) + 1
                global_step = resume_state.get('global_step', 0)
                best_dev_acc = resume_state.get('best_dev_acc', 0.0)
                final_test_acc = resume_state.get('final_test_acc', 0.0)
                best_dev_epoch = resume_state.get('best_dev_epoch', 0)
                print(f'resumed at epoch {start_epoch} with best_dev_acc={best_dev_acc:.4f}, final_test_acc={final_test_acc:.4f}')
            else:
                print('checkpoint has no optimizer/epoch metadata; resuming from epoch 0 with loaded weights only')
    ...
    if start_epoch >= args.n_epochs:
        print(f'training already complete at epoch {start_epoch}; nothing to do')
        return
    ...
    # try:
        for epoch_id in range(start_epoch, args.n_epochs):
            ...
            if args.save_model:
                save_resume_checkpoint(checkpoint_last_path, model, args, epoch_id, global_step, best_dev_acc, final_test_acc, best_dev_epoch)
                print(f'resume checkpoint saved to {checkpoint_last_path}')
            model.train()
            start_time = time.time()
...
def eval_detail(args):
    ...
    model_state_dict, old_args = torch.load(model_path, map_location=torch.device('cpu'), weights_only=False)

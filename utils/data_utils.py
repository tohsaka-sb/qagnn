...
def load_sparse_adj_data_with_contextnode(adj_pk_path, max_node_num, num_choice, args):
    ...
    if use_cache:
        ...
        missing_cid2score_warned = False
            ...
            if (cid2score is not None):
                for _j_ in range(num_concept):
                    ...
                    if _cid in cid2score:
                        ...
                        # Some custom graph generators may omit context node (-1)
                        # or a few concept scores. Fall back to 0.0 to keep loading robust.
                        node_scores[idx, _j_, 0] = torch.tensor(0.0)
                        if not missing_cid2score_warned:
                            print(f'[warn] missing cid2score for cid={_cid}; fallback to 0.0')
                            missing_cid2score_warned = True

...
def match_mentioned_concepts(sents, answers, num_processes, total=None, initial=0):
    if total is None:
        total = len(sents)
    with Pool(num_processes) as p:
        yield from tqdm(p.imap(ground_qa_pair, zip(sents, answers)), total=total, initial=initial)


# To-do: examine prune
def load_cpnet_vocab_for_pruning(cpnet_vocab_path):
    with open(cpnet_vocab_path, "r", encoding="utf8") as fin:
        return set(l.strip() for l in fin)


def prune_item(item, cpnet_vocab):
    qc = item["qc"]
    prune_qc = []
    for c in qc:
...
def prune(data, cpnet_vocab_path):
    cpnet_vocab = load_cpnet_vocab_for_pruning(cpnet_vocab_path)
    prune_data = []
    for item in tqdm(data):
        prune_data.append(prune_item(item, cpnet_vocab))
    return prune_data
...
def ground(statement_path, cpnet_vocab_path, pattern_path, output_path, num_processes=1, debug=False):
    ...
    if os.path.exists(output_path) and not debug:
        with open(output_path, 'r', encoding='utf-8') as fin:
            completed = sum(1 for line in fin if line.strip())
        if completed > len(sents):
            raise ValueError(f'{output_path} has {completed} rows, expected at most {len(sents)}.')
        if completed == len(sents):
            print(f'grounded concepts already exist at {output_path} ({completed}/{len(sents)} rows).')
            print()
            return
        if completed > 0:
            print(f'resuming grounding from existing output: {completed}/{len(sents)} rows in {output_path}')

    remaining_sents = sents[completed:]
    remaining_answers = answers[completed:]

    # Stream results to disk so long-running grounding can be resumed safely.
    mode = 'a' if completed > 0 and not debug else 'w'
    with open(output_path, mode, encoding='utf-8') as fout:
        for idx, dic in enumerate(
            match_mentioned_concepts(
                remaining_sents,
                remaining_answers,
                num_processes,
                total=len(sents),
            ...
            fout.write(json.dumps(prune_item(dic, cpnet_vocab)) + '\n')
            if idx % 100 == 0:
                fout.flush()
